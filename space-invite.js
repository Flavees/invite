...
mongoose.connect('mongodb+srv://invite:123invitebot@cluster0.1gvyo.mongodb.net/invite?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect linki
  ...
  if (isMemberFake) {
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`${member} katıldı! **Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** davet <a:lev1:780015519456559105>)`).catch(err => {});
        });
      } else {
        inviterData.fake++
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
        if (!inviterData) {
          ...
          newInviter.save().then(x => {
            if (inviteChannel) inviteChannel.send(`${member} katıldı! **Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** davet <a:lev:780015478486597643> )`).catch(err => {});
          });
        } else {
          inviterData.regular++;
          inviterData.save().then(x => {
...
client.on("message", async message => {
  ...
  if (command === "davet" || command === "invites" || command === "invites") {
